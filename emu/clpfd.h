...
#define SOUND_U_ADD_U(u1, u2)                                                  \
  ((u1 >= BP_MAXINT_1W || u2 >= BP_MAXINT_1W) ? BP_MAXINT_1W : u1 + u2)
#define SOUND_L_ADD_L(l1, l2)                                                  \
  ((l1 <= BP_MININT_1W || l2 <= BP_MININT_1W) ? BP_MININT_1W : l1 + l2)

#define SOUND_L_SUB_U(l, u)                                                    \
  ((l <= BP_MININT_1W || u >= BP_MAXINT_1W) ? BP_MININT_1W : l - u)
#define SOUND_U_SUB_L(u, l)                                                    \
  ((l <= BP_MININT_1W || u >= BP_MAXINT_1W) ? BP_MAXINT_1W : u - l)
...
#define IS_SMALL_DOMAIN(dv_ptr)                                                \
  (DV_first(dv_ptr) >= fd_region_low && DV_last(dv_ptr) <= fd_region_up)

#define IS_SMALL_DOMAIN2(min, max) (min >= fd_region_low && max <= fd_region_up)

#define CALL_COUNT_DOMAIN_ELMS(dv_ptr, from, to)                               \
  (from > to ? 0 : count_domain_elms(dv_ptr, from, to))

#define CALL_EXCLUDE_DOMAIN_ELMS(dv_ptr, from, to)                             \
  (from > to ? 0 : exclude_domain_elms(dv_ptr, from, to))
...
#define CALL_DOMAIN_REGION_MIN_MAX(dv_ptr, min, max)                           \
  {                                                                            \
    if (min == max) {                                                          \
      if (IS_BV_DOMAIN(dv_ptr) && !dm_true(dv_ptr, min))                       \
        RETURN_0;                                                              \
      dv_ptr_first = min;                                                      \
      dv_ptr_last = max;                                                       \
      ...
      size = max - min + 1;                                                    \
      UPDATE_FIRST_LAST_SIZE(dv_ptr, dv_ptr_first, min, dv_ptr_last, max,      \
                             size);                                            \
...
#define REDUCE_DOMAIN_GE_p(a, x, l1, u1, l2)                                   \
  {                                                                            \
    ...
    if (IS_SUSP_VAR(x)) {                                                      \
      ...
      dv_ptr_last = DV_last(dv_ptr);                                           \
      SOUND_UP_DIV_L(first0, first, a);                                        \
      if (first0 > dv_ptr_first)                                               \
        CALL_DOMAIN_REGION_MIN(dv_ptr, first0);                                \
      temp_i = SOUND_C_MUL_U(a, dv_ptr_last);                                  \
      first = SOUND_L_SUB_U(l2, temp_i);                                       \
    } else {                                                                   \
      x = a * INTVAL(x);                                                       \
  ...
  {                                                                            \
    ...
    if (IS_SUSP_VAR(x)) {                                                      \
      ...
      if (first > dv_ptr_first)                                                \
        CALL_DOMAIN_REGION_MIN(dv_ptr, first);                                 \
      first = SOUND_L_SUB_U(l2, dv_ptr_last);                                  \
    } else {                                                                   \
      x = INTVAL(x);                                                           \
      if (x < first)                                                           \
...
#define REDUCE_DOMAIN_GE_m(a, x, l1, u1, l2)                                   \
  {                                                                            \
    ...
    if (IS_SUSP_VAR(x)) {                                                      \
      ...
      dv_ptr_last = DV_last(dv_ptr);                                           \
      SOUND_LOW_DIV_U(last0, last, a);                                         \
      if (last0 < dv_ptr_last)                                                 \
        CALL_DOMAIN_REGION_MAX(dv_ptr, last0);                                 \
      temp_i = SOUND_C_MUL_L(a, dv_ptr_first);                                 \
      first = SOUND_L_ADD_L(l2, temp_i);                                       \
    } else {                                                                   \
      x = a * INTVAL(x);                                                       \
...
#define REDUCE_DOMAIN_GE_m1(x, l1, u1, l2)                                     \
  {                                                                            \
    ...
    if (IS_SUSP_VAR(x)) {                                                      \
      ...
      dv_ptr_last = DV_last(dv_ptr);                                           \
      if (last < dv_ptr_last)                                                  \
        CALL_DOMAIN_REGION_MAX(dv_ptr, last);                                  \
      first = SOUND_L_ADD_L(l2, dv_ptr_first);                                 \
    } else {                                                                   \
      x = INTVAL(x);                                                           \
      if (x > last)                                                            \
