...
#ifdef WIN32
  sym_ptr = insert_sym("windows",7,0);
#else
  sym_ptr = insert_sym("unix",4,0);  
#endif
  return unify(ARG(1,1),ADDTAG(sym_ptr,ATM));
}
...
int bp_compare(BPLONG val1, BPLONG val2)
{
 ...
 l_start:
  ...
  SWITCH_OP(val1,lcompare1,
			{if (ISREF(val2)) return (val1 - val2); else return -1L;},
...
int b_FAST_COMPARE_fcc(BPLONG res, BPLONG t1, BPLONG t2)
{
  /* t1, t2: two terms to be compared; 
	 res = '<' if t1<t2;
	 res = '>' if t1>t2;
...
/* t1>t2 return positive; t1=t2 return 0; t1<t2 return negative */
int bp_fast_compare(BPLONG t1, BPLONG t2)
{
  BPLONG_PTR        top;

 l_start:
  DEREF(t2);
  SWITCH_OP(t1,lcompare1,
			{if (ISREF(t2)) return (t1 - t2); else return -1L;},
			
			{SWITCH_OP(t2,lcompare2,
					   {return 1;}, 
					   {return (BPULONG)t1-(BPULONG)t2;}, 
					   {return -1L;}, 
					   {return -1L;},
					   {return 1;});},
			...
			{if (!ISLIST(t2)) 
				...
				UNTAG_ADDR(t2);
				if (t1==t2) return 0;
				res = bp_fast_compare(FOLLOW(t1), FOLLOW(t2));
				if (res){
				  return res;
				} else {
			...
			{SWITCH_OP(t2,lcompare3,
					   {return 1;},
					   {return 1;},
					   {return -1L;},
					   {goto fast_compare_struct_struct;},
					   {return 1;});},

			{SWITCH_OP(t2,lcompare4,
					   {return (t1-t2);},
					   {return -1L;},
					   {return -1L;},
					   {return -1L;},
 ...
 fast_compare_struct_struct:{
	SYM_REC_PTR       sym_ptr1, sym_ptr2;

	if (t1 == t2) return 0;
	UNTAG_ADDR(t1);
	UNTAG_ADDR(t2);
	sym_ptr1 = (SYM_REC_PTR)FOLLOW(t1);
	sym_ptr2 = (SYM_REC_PTR)FOLLOW(t2);
	if (sym_ptr1 == sym_ptr2){
	  int arity;
	  int i,res;
	  arity  = GET_ARITY(sym_ptr1);
	  for (i = 1; i <= arity; i++) {
		res = bp_fast_compare(FOLLOW(((BPLONG_PTR)t1)+i),
							  FOLLOW(((BPLONG_PTR)t2)+i));
		if (res) return res;
	  }
	  return 0;
	} else {
	  return (BPULONG)sym_ptr1 - (BPULONG)sym_ptr2;
	}
  }
}
...
// Improved by Neng-Fa Zhou for efficiently sorting arrays that contain duplicated values.
...
void qsort_int_array(BPLONG_PTR arr, BPLONG len) {
  ...
  while (i >= 0) {
    ...
    if (L < R) {
      piv = arr[L];
      while (L < R) {
        while (L < R && arr[R] > piv) R--; arr[L++] = arr[R];
        while (L < R && arr[L] <= piv) L++; if (L < R) arr[R--] = arr[L]; 
      }
      arr[R] = piv; 
	  L = R-1;
...
void qsort_term_array(BPLONG_PTR arr, BPLONG len) {
  ...
  while (i >= 0) {
    ...
    if (L < R) {
      piv = arr[L];
      while (L < R) {
        while (L < R && bp_compare(arr[R],piv) > 0) R--; arr[L++] = arr[R];
        while (L < R && bp_compare(arr[L],piv)<=0) L++; if (L < R) arr[R--] = arr[L]; 
      }
      arr[R] = piv; 
	  L = R-1;
	  while (L >= beg[i] && arr[L] == piv) L--;  /* use ==, but not bp_compare, is intentional */
	  beg[i+1] = R+1; end[i+1] = end[i]; end[i++] = L+1;
      if (end[i]-beg[i]>end[i-1]-beg[i-1]) {
        swap = beg[i]; beg[i] = beg[i-1]; beg[i-1] = swap;
        swap = end[i]; end[i] = end[i-1]; end[i-1] = swap; 
